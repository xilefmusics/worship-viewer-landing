          ...
          {tutorial.sections.map((section) => (
            <Card key={section.id} id={section.id} className="scroll-mt-24">
              ...
              <CardContent className="space-y-6">
                {(() => {
                  // Group consecutive guitarTab items together
                  const groupedContent: Array<{
                    type: "group" | "single";
                    items: typeof section.content;
                    startIndex: number;
                  }> = [];
                  
                  let currentGroup: typeof section.content = [];
                  let groupStartIndex = 0;
                  
                  section.content.forEach((item, index) => {
                    if (item.type === "guitarTab") {
                  ...
                  return groupedContent.map((group, groupIndex) => {
                    if (group.type === "group") {
                      // Render multiple guitar tabs in a responsive flex container
                      return (
                        <div
